...
- **Never run `npx expo start` directly in a shell.** Always use `restartWorkflow({ workflowName: "Start Frontend" })` to restart Metro. The workflow process has required environment variables (`PORT`) injected automatically — these are not available in a bare shell.
...
- NEVER run `npx expo start` or `npx expo` directly in a shell. Use `restartWorkflow({ workflowName: "Start Frontend" })` instead — running expo directly will miss environment variables injected into the workflow.
